...
export class EmployeeRepository extends BaseRepository<Employee> {
  ...
  getPaginated = async (page: number, limit: number) => {
    ...
    const { rows } = await this.db.query<EmployeeRecord>(`select e.id, concat(e.first_name, ' ', e.second_name, ' ', e.first_surname, ' ', e.second_surname) as name, 
      c."name" as country, it."name" as "IDType", identification_number "identificationNumber", e.email, entry_date "entryDate", a."name" as area, 
      e.state, e.registration_date "registrationDate", e.update_date "updateDate"
      from employees e
